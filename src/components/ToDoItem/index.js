...
const ToDoItem = ({ eachTodo, onEditTodo, onDeleteTodo, onToggleComplete }) => {
  // Destructure eachTodo object to get id, text, and completed properties
  const { id, text, completed } = eachTodo;

  // State to control the edit mode
  const [isEditing, setIsEditing] = useState(false);

  // State to store the new text for editing
  ...
  return (
    <li className={`todo-item ${completed ? "completed" : ""}`}>
      ...
      <div className='edit-delete-sec'>
        ...
        <button className='del-button' onClick={() => onDeleteTodo(id)}>
          <svg
